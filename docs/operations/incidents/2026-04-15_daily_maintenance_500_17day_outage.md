...
**検知メカニズム自体は機能していたが、「検知 → 対応」フローが17日間断絶していた。**

| 時刻 (UTC) | イベント |
|---|---|
| 2026-03-21 | `ci-failure-notifier.yml` 作成（検知メカニズム整備） |
| 2026-03-27 15:02 | Daily Maintenance 初回 500 失敗（Issue 起票なし。原因は不明） |
| **2026-03-29 15:23:45** | **Issue #2 "CI: Daily Maintenance failed" 自動起票 ← 実際の検知タイミング** |
| 2026-03-29 〜 2026-04-14 | **17日間 Issue #2 が open・コメント0件のまま放置**（ci-failure-notifier は同名 open Issue があると追加起票しないため、17回分の失敗が1 Issueに集約） |
| 2026-04-14 頃 | 人間が「日次 GitHub Actions が失敗しているログがある」と Issue #2 に気付く |
...
| 2026-04-15 | Step 6 で別 FK エラーが順次露見（TASK-384 で 00045 検討中） |

**発見トリガーは「人間が既存 Issue に気付いたこと」。** 仕組みによる自動起票は成立したが、Issue が通知される先（Slack/Discord 等）や AI セッション開始時のチェックに組み込まれていなかったため、**起票から発見まで17日間かかった**。
...
- **Issue 監視の仕組み化:** ci-failure の Issue が open のまま放置されないよう、**AI セッション開始時のチェック項目に `gh issue list --label ci-failure --state open` を必須化**（オーケストレーター AI の「状況把握」ステップに追加）
- **PR レビューチェックリスト:** 新規 FK 追加時に「ON DELETE 句が明示されているか」「コード側で削除するテーブルか」を確認

### Q8. 今後の再発防止策

「検出」と「防止」を区別し、**防止 > 検出** の優先順位で列挙する。

#### 防止（構造変更）— 優先度高

1. **AI セッション開始時の CI Issue 確認を必須化**
   - オーケストレーター AI の「スプリントサイクル ステップ1: 状況把握」に `gh issue list --label ci-failure --state open` を追加
   - 17日放置の再発防止に直結。**本インシデントの最も重要な教訓**
2. **schema 規約の明文化**
   - `.claude/rules/database-schema.md`（新設）に以下を追加:
     - 「`REFERENCES` 句には `ON DELETE {CASCADE|SET NULL|RESTRICT}` を必須明示」
     - 「RPC パラメータ型と UPDATE/INSERT 対象列の型が異なる場合、`::type` 明示キャスト必須」

#### 検出（テスト追加）— 優先度中

3. **`schema-consistency.test.ts` の拡張**
   - PostgreSQL `pg_constraint.confdeltype` を検証し、全 FK に `ON DELETE` 句が明示されていることを確認
4. **`performDailyReset` integration test の新設**
   - Supabase Local で撃破済みチュートリアルBOT + 関連4テーブルを仕込んだ後、`deleteEliminatedTutorialBots()` + 全 Step の E2E 成功を検証

#### 検出（運用監視）— 優先度中

5. **`bdd-smoke` への internal API トリガ追加**
...
「検知メカニズムは整備されているが、通知先が unmonitored な場所（GitHub Issue のみ）」という構造は他の監視にも当てはまる可能性。Cloudflare Observability 既知エラー（sprint_current.md `CF Observability 既知エラー` 節）も同様に「ログはあるが人が定期的に見ていない」状態の延長。**能動的な通知ルート（Slack/Discord/AIセッション起点チェック）への統合**を中長期で検討。
...
- ✅ 原因 B の1/4（`bot_posts`）: migration 00044 で解消、本番適用済み
- ⏳ 原因 B の3/4（`attacks` / `grass_reactions` / `collected_topics`）: TASK-384（migration 00045）人間承認待ち
- ⏳ 本番実機検証: `gh workflow run daily-maintenance.yml` で daily-reset + daily-stats ジョブ PASS 確認
- ⏳ GitHub Issue #2 クローズ: 全修正完了 + 実機検証 PASS 後
- ⏳ 再発防止策の実装: 次スプリント以降で段階的に
  - 高優先度: AI セッション開始時の CI Issue 確認
  - 高優先度: schema 規約の明文化
...
- 関連教訓: LL-002（InMemoryリポジトリは実DBの制約を再現する）
- 関連 GitHub Issue: #2
