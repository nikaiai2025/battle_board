...
| # | 方法 | 実現性 | 備考 |
|---|---|---|---|
| A | **Cloudflare Workers移行** | 高 | 「Always Use HTTPS」OFF でHTTP:80直接応答可能 |
...
### 推奨: 選択肢A（Cloudflare Workers移行） → 採用済み

`@opennextjs/cloudflare` を使用して Cloudflare Workers にデプロイ済み（`battle-board.shika.workers.dev`）。HTTP:80での直接応答が可能になり、ChMateの接続問題は解消された。

2026-03-16の追加診断で、bbsmenu.html に `https://` を記載してもChMateは `http://` にダウングレードすることを確認。詳細は `docs/research/senbra_protocol_diagnosis_2026-03-16.md` を参照。
...
| HTTP/HTTPSプロトコル診断レポート (2026-03-16) | `docs/research/senbra_protocol_diagnosis_2026-03-16.md` |
