...
入力は UTF-8の `Post[]`。出力は Shift_JIS エンコード前の DAT テキスト（1レス=1行、`<br>` 改行）。ShiftJisEncoderへの変換は呼び出し元（Route Handler）が行う。
...
BbsCgiParsedRequest {
  threadKey:   string
  boardId:     string
  message:     string   // Shift_JISデコード済み
  name:        string   // 同上
  mail:        string   // 同上
  edgeToken:   string | null  // cookieから取得
}
```
...
```
buildSuccess(threadKey: string, boardId: string): string   // HTML
buildError(message: string): string                        // HTML
...
| ThreadRepository | スレッド一覧取得（subject.txt）、dat_byte_size参照 |
