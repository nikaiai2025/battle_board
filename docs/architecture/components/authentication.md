...
> ステータス: ドラフト / 2026-03-14 (Sprint-17 認証フロー是正 G1〜G4 対応)
> 関連D-07: § 5 認証アーキテクチャ
...
- `not_verified`: edge-token は存在するが `is_verified=false`（認証コード未検証）。G1対応。
...
```
verifyAuthCode(code: string, turnstileToken: string, ipHash: string): VerifyAuthCodeResult
```
```
...
| コンポーネント | 依存の性質 |
|---|---|
| AuthCodeRepository | 認証コードの保存・検索・更新（verified, write_token, write_token_expires_at） |
...
`users.is_verified` フラグにより、edge-tokenが存在しても認証コード検証が完了していない場合は書き込みを拒否する。フラグは `verifyAuthCode` 成功時または `verifyWriteToken` 成功時に `true` に更新される。
...
認証フロー:
1. ユーザーが Web UI の `/auth/verify` で Turnstile + 認証コードを検証する
2. 検証成功時に `verifyAuthCode` が write_token（32文字 hex）を生成して返却する
