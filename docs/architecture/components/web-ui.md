...
**ポーリング方式**: 定期的な `GET /api/threads/{threadId}/posts?since={lastPostNumber}` で新着レスを取得。WebSocketは使用しない（Serverless環境の制約・初期フェーズでは不要）。`PostListLiveWrapper` に `pollingEnabled` props を追加し、過去ページ（range 指定で末尾レス番号 < postCount）では `false` を渡してポーリングを無効化する。
...
- 旧 `threads/[threadId]/page.tsx` はリダイレクト専用に変更済み（UUID→新URLへの302リダイレクト）
