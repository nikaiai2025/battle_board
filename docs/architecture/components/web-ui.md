...
> 関連D-08: authentication.md / user-registration.md

> NOTE:
> 本改訂案は、専ブラ認証後の通常ブラウザ導線を Web UI 側で破綻させないための責務整理である。
> Web UI は「認証済みかどうか」だけでなく、「その token が `web` channel かどうか」まで見て表示制御しなければならない。
...
| マイページ（残高・履歴） | SSR（`web` channel 認証チェック後） | ユーザー固有情報のため `web` edge-token が必須。CSRでの表示は行わない |
        ...
        └── PostFormContextProvider [Client Component]  // レス番号クリック → PostForm テキスト挿入 Context
              └── PostList [Client Component]           // 初期レス一覧（Context消費のためClient化）
                    └── PostItem [Client Component]     // 1レスの表示（レス番号クリック→insertText）
                          ...
                          └── ImageThumbnail [Client Component]  // 画像URLサムネイル表示（@image_preview）
              └── PostListLiveWrapper [Client Component]     // ポーリングで新着取得
                    └── PostItem [Client Component]
              └── FloatingActionMenu [Client Component]      // FAB + ボトムシート（@fab）
                    └── PostForm [Client Component]          // 書き込みフォーム（常時マウント、CSS開閉）
                          └── AuthModal [Client Component]   // Turnstile認証（未認証時）
                    └── Sheet(検索) [Client Component]       // 未実装（モック）
                    └── Sheet(画像) [Client Component]       // 未実装（モック）
                    └── Sheet(設定) [Client Component]       // 未実装（モック）
...
- `FloatingActionMenu` 追加（FAB + ボトムシート。書き込み・検索・画像・設定。See: features/thread.feature @fab）
- `PostForm` の配置を直接配置から `FloatingActionMenu` 内に移動（常時マウント + CSS translate-y で開閉）
...
app/(web)/mypage/page.tsx  [Server Component]
  ...
  └── RegistrationPanel // 本登録導線
  └── PatPanel          // #pat_<token> 表示・再発行
```
...
### 4.1 専ブラ認証リンクを通常ブラウザで開いた場合

1. 専ブラが返した `/auth/verify?token=<senbra-edge-token>` を通常ブラウザで開く
2. Turnstile 通過後、`/api/auth/verify` が同一 `user_id` の `web` token を発行する
3. Web UI は以後その `web` token を前提に、`/mypage` または本登録導線へ遷移する

UI 側の重要事項:
- `senbra` token のままでは `mypage` への導線を出してはならない
- Header / Layout の認証表示は `isAuthenticated` だけでなく `channel === 'web'` を見て判定する
- `web` token へ正規化される前の状態では、「認証済み」と「Web の会員機能が利用可能」は同義ではない
...
### Header / Layout は channel-aware にする

Web UI の共通レイアウトは、単に Cookie の有無だけでログイン状態を表示してはならない。`senbra` token を通常ブラウザに持ち込んだ直後は、投稿主体としては認証済みでも、Web 会員機能の利用資格はまだ `web` channel に正規化されていない可能性があるためである。

したがって Header / Layout は少なくとも以下を区別する。
- 未認証
- 認証済みだが `senbra` channel
- 認証済みかつ `web` channel

`mypage`、本登録、PAT 管理、設定変更などの導線は `web` channel のときのみ表示対象とする。
