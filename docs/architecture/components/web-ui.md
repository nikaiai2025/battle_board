...
Web UIはビジネスロジックを持たない。データ取得・変更はすべてAPIルート（自身の `app/api/` 配下）を経由し、サービス層を直接インポートしない。
...
| 画面・操作 | 方式 | 理由 |
|---|---|---|
| スレッド一覧表示 | SSR (Server Component) | SEO・初期表示速度。PostServiceを直接呼ぶのではなくAPIルート経由でfetch |
...
### Server ComponentからAPIルートを呼び出す理由

Server Componentからサービス層を直接importすることは技術的には可能だが、以下の理由でAPIルート経由のfetchを採用する。
...
データ取得：`GET /api/threads?boardId=battleboard`（APIルート経由）
...
Web UIコンポーネントはすべてAPIルートのみに依存する。サービス層・ドメイン層を直接importしない。
