...
コマンド実行結果・書き込み報酬など、**投稿時点で即座に確定する情報**は投稿者のレス本文末尾に区切り線付きで付加する。
...
#### 方式B: 独立システムレス（非同期・イベント型）

特定の投稿に紐づかないイベントや、**後から発生するイベント**は「★システム」名義の独立レスとしてPostServiceが挿入する。
...
| 条件 | 方式 |
|---|---|
| ユーザーの投稿をトリガーとし、同一トランザクション内で結果が確定する | A（レス内マージ） |
| ユーザーの投稿に紐づかない、または非同期に発生する | B（独立システムレス） |

CommandServiceは方式を意識しない。`CommandExecutionResult.systemMessage` として文字列を返すのみ。方式の判断と実際のDB操作はPostServiceが担う。
