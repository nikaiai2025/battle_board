...
> NOTE:
> 本改訂案は、専ブラ認証後の Web 本登録導線断絶と、Cookie 分岐時の PAT 復帰不全に対する設計修正である。
> 現行 D-03 と衝突する箇所を含むため、実装前に `features/user_registration.feature` および必要に応じて `features/specialist_browser_compat.feature` の更新承認を前提とする。
...
| **token channel** | edge-token の発行経路。`web` は Web UI 用、`senbra` は専ブラ互換投稿用 | `edge_tokens.channel` |
...
CREATE TABLE edge_tokens (
  ...
  channel      VARCHAR NOT NULL, -- 'web' | 'senbra'
...
```sql
-- 既存データの移行
INSERT INTO edge_tokens (user_id, token, channel, created_at, last_used_at)
SELECT id, auth_token, 'web', created_at, NOW()
FROM users
...
移行時の `channel` 既定値は `web` とする。旧 `users.auth_token` は channel を持たないため厳密な復元は不可能であり、通常ブラウザ導線の継続性を優先する。専ブラ側は以後の認証・PAT・write_token フローで `senbra` token を取得する。
...
│ pat_token(UQ)│       │  │ channel      │
│ pat_last_    │       │  │ created_at   │
...
補足:
- `edge_tokens.channel='web'` は Web UI のマイページ・設定変更・PAT 管理を許可する
- `edge_tokens.channel='senbra'` は専ブラ互換投稿系に限定する
- 通常ブラウザで `/auth/verify?token=<senbra-edge-token>` を完了した場合は、同一 `user_id` に対して `web` channel の edge-token を追加発行し、ブラウザ Cookie を `web` token に正規化する
...
```
PatLoginResult:
  | { success: true; userId: UUID; edgeToken: string; reusedCurrentToken: true }
  | { success: true; userId: UUID; edgeToken: string; reusedCurrentToken: false }
  | { success: false; reason: "invalid_pat" }
```

処理:
1. `verifyPat()` で PAT の所有者を特定する
2. `currentEdgeToken` が存在する場合は `verifyEdgeToken()` 相当の参照で所有者を確認する
3. `currentEdgeToken` が PAT 所有者と同一ユーザーに属していれば、その token を再利用する
4. `currentEdgeToken` が欠落・失効・別ユーザー所属のいずれかであれば、PAT 所有者に対して新しい `senbra` channel の edge-token を発行する
5. 呼び出し元の bbs.cgi ルートは、返却された edge-token で Cookie を上書きする

設計意図:
- PAT が入力された場合は、専ブラ投稿時の本人性主張として PAT を最優先で扱う
- Cookie は PAT 所有者と矛盾しない場合にのみ再利用する
- PAT が無効な場合は、既存 Cookie が有効でも PAT 入力をエラーとして扱う
...
```sql
SELECT et.user_id, et.channel, u.author_id_seed, u.is_verified
FROM edge_tokens et
...
書き込みリクエスト到着
    ...
    │     YES → loginWithPat() を実行
    │             → PAT 有効 + Cookie 同一ユーザー → Cookie 維持 → 認証OK
    │             → PAT 有効 + Cookie 欠落/失効/別ユーザー → 新 senbra token 発行
    │                     → Cookie 上書き → 認証OK
    │             → PAT 無効 → エラー応答
    │     NO → ②へ
    │
...
※ PAT / write_token が mail 欄に含まれていた場合は、認証成否にかかわらず
   PostService に渡す前に除去する（DAT 漏洩防止）
...
### 7.0 専ブラ認証リンクから通常ブラウザへ遷移した場合

主ケースは、専ブラ投稿時に返された認証 URL を通常ブラウザで開き、Turnstile を通過してから本登録へ進む流れである。

```
専ブラで未認証投稿
    │
    │  bbs.cgi が /auth/verify?token=<senbra-edge-token> を返す
    ▼
通常ブラウザで /auth/verify?token=... を開く
    │
    │  Turnstile 通過
    ▼
┌────────────────────────────────────────────┐
│ POST /api/auth/verify                      │
│ ① query の senbra edge-token を検証        │
│ ② 同一 user_id に対して新しい web token を発行 │
│ ③ ブラウザ Cookie を web token で上書き      │
│ ④ redirect=/mypage または register へ遷移    │
└────────────────────────────────────────────┘
```

この正規化により、通常ブラウザは以後 `web` channel として扱われ、マイページ・本登録導線へ自然接続できる。
`senbra` token をブラウザ Cookie に残したまま Web UI を継続利用することは許可しない。

### 7.1 メール認証

```
仮ユーザー（web edge-token 認証済み）
    │
    │  マイページ → 「本登録」
    │  メールアドレス + パスワード入力
    ▼
...
PAT はマイページに常時表示する。ただし表示文字列は専ブラにそのまま貼り付け可能な `#pat_<token>` のみとし、素の token 単体表示は行わない。
...
PAT は mail 欄に設定して使用する。PAT が入力されている場合は、その書き込みにおける本人性主張の正本として扱う。

| Cookie 状態 | mail 欄 | 動作 |
|---|---|---|
| 同一ユーザーの有効 Cookie | `sage#pat_xxx` | PAT 所有者と Cookie 所有者が一致。Cookie を再利用して認証 |
| 別ユーザーの有効 Cookie | `sage#pat_xxx` | PAT を正として認証。PAT 所有者向けの新 Cookie を発行し上書き |
| 失効/なし | `sage#pat_xxx` | PAT を正として認証。PAT 所有者向けの新 Cookie を発行 |
| 任意 | `sage#pat_invalid` | エラー。無効 PAT のため書き込み拒否 |
...
| **/api/auth/verify** | `senbra` token をそのままブラウザ保持する方式を廃止 | Turnstile 成功後に同一 user_id の `web` token を発行し Cookie を正規化 |
| **bbs.cgi ルート** | mail 欄パースに `#pat_` パターン追加。PAT が Cookie より優先 | PAT 検出を最優先にし、Cookie 競合時は PAT 所有者へ上書き |
| **マイページ** | 本登録セクション・PAT セクション追加、課金ボタンにガード追加 | UI 拡張。PAT 表示は `#pat_<token>` のみ |
| **Web Layout / Header** | 認証済みでも `senbra` token の場合は Web 導線を出してはならない | 認証状態を channel-aware に拡張 |
...
本登録は既存の `users` レコードに `supabase_auth_id` を紐付けるだけで、原則として既存の `web` edge-token を再発行・無効化しない。ユーザーが意識せずにシームレスに移行できる。

例外は、`senbra -> web` の正規化を `/api/auth/verify` で実施するケースである。これは本登録の副作用ではなく、通常ブラウザ導線に入る前提条件として扱う。

### 通常ブラウザで使う edge-token は `web` channel に正規化する

`senbra` token を通常ブラウザへ持ち込めること自体は、専ブラ認証リンクの成立に必要である。ただし、その token をブラウザ上で継続利用すると `mypage` と UI ナビゲーションが不整合になる。

このため `/api/auth/verify` を、`senbra -> web` の唯一の昇格ポイントとする。Turnstile 成功後は同一 `user_id` に対する新しい `web` token を発行し、ブラウザ Cookie を正規化する。

### PAT は専ブラ投稿時の本人性主張の正本として扱う

PAT は「Cookie が壊れたときの保険」だけでは不十分である。Cookie が別ユーザーに分岐した場合、Cookie 優先では本来のアカウントに復帰できない。

そのため専ブラ投稿時に PAT が入力された場合は、PAT をその書き込みにおける本人性主張の正本とする。既存 Cookie は PAT 所有者と一致する場合にのみ再利用し、不一致なら上書きする。

### 本改訂案は D-03 更新を前提とする

現行 `features/user_registration.feature` は、Cookie 優先・PAT 非優先の受け入れ基準を含んでいる。本書の改訂案はその振る舞いを変更するため、feature 承認前に実装してはならない。
