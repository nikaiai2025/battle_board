...
Phase 1-2 の「仮ユーザー」（edge-token + 認証コードのみ）に加え、メールアドレスまたは Discord アカウントによる「本登録」を導入する。本登録により Cookie 喪失・端末変更時にログインで同一ユーザーに復帰でき、アイデンティティの永続化を実現する。
...
| 用語 | 定義 | DB上の判定 |
|---|---|---|
| **仮ユーザー** | edge-token + 認証コードで認証済み、Supabase Auth 未連携 | `users.supabase_auth_id IS NULL` |
...
```
  ...
  └──────┬───────┘
         ...
         ▼
  ┌──────────────┐
  │ 認証コード    │ ← edge-token 発行済み, is_verified=false
  │ 発行済み      │
  └──────┬───────┘
...
書き込みリクエスト到着
    ...
    │             → 見つかった + is_verified=false → 認証コード案内
    │             → 見つからない → ②へ
    │     NO → ②へ
    │
...
| 失効/なし | `sage` | 未認証。認証コード案内（本登録ユーザーでもログインが必要） |
...
| AuthCodeRepository | 認証コード管理（既存） | 既存 |
