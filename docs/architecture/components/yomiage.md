...
> 関連feature: `features/ドラフト_実装禁止/command_yomiage.feature`（承認待ち v2）
...
| 非同期 | GitHub Actions (`scripts/yomiage-worker.ts`) | pending 取得 → Gemini TTS 呼び出し → WAVヘッダ付与 → 軽量化 → 音声配信ストレージアップロード → 結果送信 |
...
| ffmpeg (CLI) | WAV の軽量化（GH Actions worker 内のみ。`ubuntu-latest` ランナーに同梱済み） |
...
2. 各 pending について下記を実行:
   ...
   4. ffmpeg で軽量化（§5.5 参照）
   5. `audioStorageAdapter.upload(wavBuffer)` でアップロードし URL 取得（§5.6 参照）
...
**依存**: `ubuntu-latest` GH Actions ランナーに同梱される `ffmpeg` CLI を `child_process.spawn` 経由で呼び出す
**軽量化設定**:
- サンプルレート 24000 Hz 維持
- モノラル維持
- ビットレートを 16bit PCM から 64kbps AAC に変換、または MP3 128kbps（どちらも配信側の互換性が高い）
- ファイル拡張子は `.wav` のまま配布するかは §5.6 で検討（ストレージ側の仕様に依存）

> **検討事項**: feature 上の契約は「URLが指すファイルは WAV 形式である」。ffmpeg で AAC/MP3 変換すると feature 違反になるため、軽量化は **WAV の範囲内（PCM 16bit → 8bit or サンプルレート下げ）** で行う。feature が「WAV 形式」ではなく「音声ファイル」で良いなら形式自由度が上がるが、スコープ変更は人間承認が必要。
>
> **推奨（初版）**: WAV 形式を維持する方針とし、feature に従う。軽量化はサンプルレート 16kHz へのダウンサンプル + 16bit 維持でファイルサイズを約 2/3 にする。
...
```
>>5 の読み上げ音声ができたよ
https://litterbox.catbox.moe/xxxxxxxx.wav
...
### 9.5 feature を `features/ドラフト_実装禁止/` に配置したまま承認ゲートを通す

- 本feature は非同期コマンド・外部ストレージ連携・TTS 新導入という複合要素を含み、feature 承認と同時に本D-08設計書と TDR-018 のレビューも伴う必要がある
- feature 単独で承認せず、D-08 設計書＋feature セットで人間レビューを経て `features/` 直下へ移動する運用とする
...
.github/workflows/
  yomiage-scheduler.yml                   # workflow_dispatch 専用
...
- **エスカレーション**: `tmp/escalations/escalation_LITTERBOX_ADOPTION.md`
- **feature**: `features/ドラフト_実装禁止/command_yomiage.feature`
