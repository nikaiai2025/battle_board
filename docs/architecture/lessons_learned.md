...
死角を塞ぐために、本番環境に対する書き込みテスト（ベーシックフローテスト）を新設した。最大の障壁はTurnstile（CAPTCHA）であり、以下の3案を検討した:
...
## LL-011: UUID列に到達し得るフィールドにプレースホルダ空文字列を使わない

- **発見日:** 2026-03-21
- **発見契機:** `!attack` 撃破時のインライン表示・撃破通知・報酬付与が全てサイレント失敗していることを人間が本番手動テストで発見

### 事象

PostService はコマンド実行（Step 5）をレス INSERT（Step 9）より前に行う。コマンド実行時点ではレスが存在しないため、`postId: ""` をプレースホルダとして CommandContext に渡していた。この空文字列が AttackHandler → BotService.recordAttack → AttackRepository.create を通じて `attacks.post_id UUID NOT NULL` カラムに到達し、PostgreSQL が UUID 型構文エラーを返した。PostService の try-catch がこの例外を握りつぶし、`commandResult` が null のまま残った結果:

- インラインメッセージ（HP変化表示）: 表示されない
- 撃破通知（★システム独立レス）: 投稿されない
- 撃破報酬: 付与されない
- コスト消費・HP減少・撃破状態変更: **実行済み**（例外発生ステップより前のため）

BDD テストは AttackHandler を直接呼び出し `postId: crypto.randomUUID()` を渡すため、本番固有の UUID エラーは再現しなかった。InMemory AttackRepository も UUID バリデーションを行わないため検出不可能だった。

### 根本原因

1. **プレースホルダ空文字列**: 値が構造的に利用不可能な時点で `""` を使用し、型 `string` が空文字を許容するためコンパイラが不整合を検出できなかった
2. **サイレント失敗**: PostService の try-catch がコマンド実行全体の例外を握りつぶし、部分的な副作用実行（コスト消費あり、記録・通知なし）という不整合状態を生んだ

### 教訓

**UUID 列に到達し得るフィールドで、値が構造的に利用不可能な場合は、プレースホルダ空文字列 `""` ではなく `null` を明示的に使用する。** 受け側の型は `string | null` とし、DB スキーマも NULL 許容にする。

```typescript
// NG: 空文字プレースホルダ（型システムが検出できない時限爆弾）
commandResult = await cmdService.executeCommand({
    postId: "",  // INSERT前のためプレースホルダ
    ...
});

// OK: null を明示（意図が明確、受け側で null チェック可能）
commandResult = await cmdService.executeCommand({
    postId: null,  // INSERT前のため null
    ...
});
// 受け側: postId: string | null
```

### LL-001, LL-002 との関係

- **LL-001（ブランド型 UUID）**: ブランド型が導入されていれば `""` は `UUID` 型にキャストできずコンパイルエラーになる。LL-001 は「受け側の防御」、LL-011 は「渡す側の規律」であり、相補的
- **LL-002（InMemory 制約再現）**: InMemory repo に UUID バリデーションがあっても、BDD テストが有効な UUID を渡す経路では検出できない。LL-011 は InMemory バリデーションでは防げないケースをカバーする
...
## LL-015: 外部サービス統合はサーバーサイド向け公式パターンを最初に確認する

- **発見日:** 2026-03-25
- **発見契機:** メール確認リンクが 404 になるバグ。到達URLに `#access_token=...` が含まれ、サーバー側に到達しなかった

### 事象

Supabase の `signUp()`（Admin API）は implicit flow で動作し、メール確認後のリダイレクトに `#access_token=...`（ハッシュフラグメント）を使う。ハッシュフラグメントは HTTP リクエストでサーバーに送信されないため（RFC仕様）、サーバーサイドのコールバック処理が一切実行されなかった。

前回インシデント（LL-014）で `emailRedirectTo: undefined` を修正したが、修正時の真因検証（Q4）で「redirectTo を直せば `?code=...` が取得できる」と誤って結論づけた。implicit flow と PKCE flow の区別を理解していなかった。

### 根本原因

Supabase のクライアントサイド向けパターン（implicit flow: `#access_token`）とサーバーサイド向けパターン（PKCE flow: `?code=` / `verifyOtp()` + カスタムメールテンプレート）の区別を認識していなかった。公式ドキュメントの SSR ガイドを参照せず、クライアント向けの例を基にサーバーサイド実装を行った。

### 教訓

**外部サービス（認証プロバイダ、OAuth、決済等）との統合を設計する際は、以下の手順を踏む:**

1. **公式ドキュメントで自分のアーキテクチャに合致するガイドを探す** — 「React SPA 向け」「SSR/サーバーサイド向け」「モバイル向け」等、環境別にパターンが異なることが多い
2. **クライアントサイド向けパターンをサーバーサイドに流用しない** — リダイレクト方式（ハッシュフラグメント vs クエリパラメータ）、トークン取得方式（implicit vs PKCE）等が根本的に異なる
3. **インシデント分析の Q4（真因検証）では外部サービスの仕様レベルまで検証する** — 「コードを直せば動くはず」ではなく、外部サービスが実際にどう振る舞うかを仕様で確認する

### 本件での対策

Supabase 公式推奨の SSR パターンに移行:
- カスタムメールテンプレート: `{{ .TokenHash }}` を `/api/auth/confirm` に直接渡す
- サーバーサイド: `verifyOtp({ type, token_hash })` でトークン検証
- Supabase の `/auth/v1/verify` リダイレクト（implicit flow）を完全にバイパス

### LL-014 との関係

LL-014 と本件は同一の機能（メール本登録フロー）で連続して発生した。LL-014 の修正は正しかったが不完全であり、本件は LL-014 の Q4（真因検証）の不備が原因で見逃された。**インシデント修正時の真因検証の深さ**が、連鎖インシデントの防止に直結する。

See: `docs/operations/incidents/2026-03-25_email_confirm_implicit_flow.md`

---

## LL-016: `res.json()` の `as` 型アサーションはランタイム安全性を提供しない

- **発見日:** 2026-03-29
- **発見契機:** 本番スモークテストがマイページクラッシュを検出（インシデント `2026-03-29_mypage_vocab_response_mismatch.md`）

### 事象

新規API (`GET /api/mypage/vocabularies`) が `{ entries: [...] }` を返すのに対し、クライアントが `(await res.json()) as Array<{...}>` で bare array としてキャスト。TypeScriptコンパイラは `as` アサーションをランタイムで検証しないため、オブジェクトに対して `.map()` が呼ばれ TypeError でページ全体がクラッシュ。BDDテスト（サービス層）とvitest（単体）は全PASSしており、HTTPレスポンスの形状ミスマッチはテストピラミッドの死角だった。

### 根本原因

APIレスポンスの型定義がサーバー（route.ts）とクライアント（page.tsx）で独立して管理されており、DRY違反の構造。`as` アサーションは型チェッカーを黙らせるだけで、ランタイムでの型検証は一切行わない。

### 教訓

**`(await res.json()) as Type` は型安全に見えて実際には unsafe である。** 新規APIを追加する際は、以下のいずれかで型の整合性を保証すること:

1. **共有型定義:** APIレスポンス型を `src/types/` に定義し、サーバーとクライアントで同一型を import する
2. **レビューチェックリスト:** 最低限、APIの実レスポンス形状をクライアントの型定義と目視照合する

特に、ラッパーオブジェクト（`{ entries: [...] }`, `{ users: [...] }` 等）を返すAPIでは bare array との取り違えが起きやすい。

### 本プロジェクトでの対処

既存35箇所の全面改修はコストが高いため見送り。新規API追加時のレビューで型照合を必須化。将来的に共有型定義への段階的移行を検討。
...
<!--
LL-018 は取り下げ（2026-04-15）
初稿では「17日間の対応遅延は通知システムの認知フロー欠陥が原因」と記述したが、
事実関係の確認（人間証言）により、認知は即日成立しており別件優先の業務判断による
意図的な対応保留が実態であった。通知システムに構造的欠陥はなく、教訓化の前提が
成立しないため削除。
詳細: docs/operations/incidents/2026-04-15_daily_maintenance_500_17day_outage.md Q3
-->
