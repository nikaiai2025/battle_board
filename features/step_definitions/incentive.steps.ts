...
async function ensureNamedUser(world: BattleBoardWorld, name: string): Promise<UserContext> {
 ...
 */
BeforeStep(async function (this: BattleBoardWorld, step: ITestStepHookParameter) {
  ...
  if (stepText === 'スレッドに書き込みを1件行う' && !this.currentThreadId && this.currentUserId) {
...
Given('今日まだ書き込みをしていない', async function (this: BattleBoardWorld) {
  ...
  await InMemoryUserRepo.updateStreak(this.currentUserId, user.streakDays, null as unknown as string)
 ...
 */
Given('今日すでに1回書き込みをしている', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
  const user = await InMemoryUserRepo.findById(this.currentUserId)
  assert(user, 'ユーザーが見つかりません')
  const todayJst = getTodayJst()
  // 今日の日付で lastPostDate を設定しストリークを1日目に
  InMemoryUserRepo._insert({ ...user, lastPostDate: todayJst, streakDays: 1 })
 ...
 */
Given('今日まだスレッドを作成していない', async function (this: BattleBoardWorld) {
 ...
 */
Given('今日すでに1回スレッドを作成している', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
  const todayJst = getTodayJst()
  InMemoryIncentiveLogRepo._insert({
    id: crypto.randomUUID(),
 ...
 */
Given('今日まだ書き込みもスレッド作成もしていない', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
  const user = await InMemoryUserRepo.findById(this.currentUserId)
  if (user) {
    // lastPostDate を今日の日付に設定して daily_login の誤発火を防ぐ
 ...
 */
Given('{string} がスレッドを作成済みである', async function (
 ...
 */
Given('ユーザー {string} がスレッドを作成済みである', async function (
  this: BattleBoardWorld,
  userName: string
) {
  const ctx = await ensureNamedUser(this, userName)
 ...
 * See: features/phase1/incentive.feature @スレッドにレスが10個付き
 */
Given('そのスレッドのレス数が {int} である', async function (
  this: BattleBoardWorld,
  postCount: number
) {
 ...
 * See: features/phase1/incentive.feature @スレッドにレスが10個付き
 */
Given('そのスレッドのユニークID数が {int} 以上である', async function (
  this: BattleBoardWorld,
  minUniqueIds: number
) {
  assert(this.currentThreadId, 'スレッドが設定されていません')
 ...
 */
Given('そのスレッドのユニークID数が {int} である', async function (
  this: BattleBoardWorld,
  uniqueIds: number
) {
  assert(this.currentThreadId, 'スレッドが設定されていません')
 ...
 */
Given('ユーザー {string} がレス >>{int} を書き込み済みである', async function (
  this: BattleBoardWorld,
  userName: string,
  postNumber: number
) {
 ...
 */
Given('ユーザー {string} がレス >>{int} とレス >>{int} を書き込み済みである', async function (
  this: BattleBoardWorld,
  userName: string,
  postNumber1: number,
  postNumber2: number
 ...
 * See: features/phase1/incentive.feature @同一IDからの2回目以降の返信ではボーナスは付与されない
 */
Given('今日すでに {string} からの返信で +5 を受け取っている', async function (
  this: BattleBoardWorld,
 ...
 */
Given('レス >>{int} の書き込みから60分以上経過している', async function (
  this: BattleBoardWorld,
  postNumber: number
) {
  // UserA のコンテキストを取得
 ...
 */
Given('そのスレッドに過去書き込みをしたことがない', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
 ...
 */
Given('そのスレッドに過去書き込みをしたことがある', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
  if (!this.currentThreadId) {
    const threadKey = Math.floor(Date.now() / 1000).toString()
    const thread = await InMemoryThreadRepo.create({
 ...
 */
Given('今日すでに3つの新スレッドに初書き込みをしている', async function (
  this: BattleBoardWorld
) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
  const todayJst = getTodayJst()
...
Given('スレッドの最終レスが24時間以上前である', async function (this: BattleBoardWorld) {
  ...
  const dummyAuthorId = namedUserA ? namedUserA.userId : 'dummy-inactive-author'
  const existingPostsForInactive = await InMemoryPostRepo.findByThreadId(this.currentThreadId)
...
Given('スレッドの最終レスが12時間前である', async function (this: BattleBoardWorld) {
  ...
  const dummyAuthorIdRecent = namedUserARecent ? namedUserARecent.userId : 'dummy-recent-author'
  const existingPostsForRecent = await InMemoryPostRepo.findByThreadId(this.currentThreadId)
 ...
 */
Given(/^ユーザーが(\d+)日連続で書き込みログインボーナスを獲得済みである$/, async function (
  this: BattleBoardWorld,
  streakDaysStr: string
) {
  ...
  InMemoryUserRepo._insert({ ...user, streakDays, lastPostDate: yesterdayStr })
})
 ...
 */
Given('ユーザー {string} の通貨残高が {int} である', async function (
  this: BattleBoardWorld,
  userName: string,
  balance: number
) {
  const ctx = await ensureNamedUser(this, userName)
  InMemoryCurrencyRepo._upsert({ userId: ctx.userId, balance, updatedAt: new Date() })
})
 ...
 */
When(/^他のユーザーが(\d+)件目のレスを書き込む$/, async function (
  this: BattleBoardWorld,
  postCountTargetStr: string
) {
  ...
  const existingBalance_other = await InMemoryCurrencyRepo.findByUserId(otherCtx.userId)
  if (!existingBalance_other) {
    InMemoryCurrencyRepo._upsert({ userId: otherCtx.userId, balance: 0, updatedAt: new Date() })
  }
  ...
  this.lastResult = 'success' in result && result.success
    ? { type: 'success', data: result }
    : { type: 'error', message: 'success' in result ? result.error : '認証エラー', code: undefined }
})
 ...
 * See: features/phase1/incentive.feature @レスが10個付いてもユニークIDが3未満ならボーナスは付与されない
 */
When(/^同一ユーザーが(\d+)件目のレスを書き込む$/, async function (
  this: BattleBoardWorld,
  postCountTargetStr: string
) {
  const postCountTarget = parseInt(postCountTargetStr, 10)
  ...
  const existingPosts = await InMemoryPostRepo.findByThreadId(this.currentThreadId)
  // 既存ダミーレスの最初の dailyId を使って「同一ユーザー」を再現する
  const existingDailyId = existingPosts[0]?.dailyId ?? 'dummy-same-user-daily-id'
  const existingAuthorId = existingPosts[0]?.authorId ?? 'dummy-user-0'

  const postNumber = await InMemoryPostRepo.getNextPostNumber(this.currentThreadId)
  const newPost: Post = {
    id: crypto.randomUUID(),
    threadId: this.currentThreadId,
    postNumber,
  ...
  this.lastResult = { type: 'success', data: { success: true, postId: newPost.id, postNumber, systemMessages: [] } }
})
 ...
 * See: features/phase1/incentive.feature Rule: 他人から返信（アンカー付き）が付くと +5
 */
When('ユーザー {string} がレス >>{int} にアンカー付きで返信を書き込む', async function (
  this: BattleBoardWorld,
  userName: string,
  ...
  assert(this.currentThreadId, 'スレッドが設定されていません')
  const ctx = await ensureNamedUser(this, userName)
  const existingBalance_reply = await InMemoryCurrencyRepo.findByUserId(ctx.userId)
  if (!existingBalance_reply) {
    InMemoryCurrencyRepo._upsert({ userId: ctx.userId, balance: 0, updatedAt: new Date() })
  }
 ...
 */
When('60分以内にユーザー {string}, {string}, {string} がレス >>{int} にアンカー付きで返信する', async function (
  this: BattleBoardWorld,
  user1: string,
  user2: string,
  user3: string,
  ...
  for (const userName of [user1, user2, user3]) {
    const ctx = await ensureNamedUser(this, userName)
    const existingBalance_hot3 = await InMemoryCurrencyRepo.findByUserId(ctx.userId)
 ...
 */
When('60分以内にユーザー {string}, {string} の2人がレス >>{int} にアンカー付きで返信する', async function (
  this: BattleBoardWorld,
  user1: string,
  user2: string,
  targetPostNumber: number
  ...
  // 対象レスの作者IDを取得して reply ログを事前挿入（重複ガードで reply ボーナスを抑制）
  const allPosts2 = await InMemoryPostRepo.findByThreadId(this.currentThreadId)
  const targetPost2 = allPosts2.find(p => p.postNumber === targetPostNumber)
  const targetAuthorId2 = targetPost2?.authorId ?? null
  const todayJst2 = getTodayJst()

  for (const userName of [user1, user2]) {
    const ctx = await ensureNamedUser(this, userName)
    const existingBalance_hot2 = await InMemoryCurrencyRepo.findByUserId(ctx.userId)
 ...
 * See: features/phase1/incentive.feature @返信が60分を超えた場合はホットレスボーナスは付与されない
 */
When('3人目のユーザーがレス >>{int} にアンカー付きで返信する', async function (
  this: BattleBoardWorld,
  targetPostNumber: number
) {
  assert(this.currentThreadId, 'スレッドが設定されていません')
  ...
  // 対象レスの作者IDを取得して reply ログを事前挿入（重複ガードで reply ボーナスを抑制）
  const allPostsLate = await InMemoryPostRepo.findByThreadId(this.currentThreadId)
  const targetPostLate = allPostsLate.find(p => p.postNumber === targetPostNumber)
  ...
  for (const ctx of [user1Ctx, user2Ctx]) {
    const existingBalance_late = await InMemoryCurrencyRepo.findByUserId(ctx.userId)
  ...
  const existingBalance_late3 = await InMemoryCurrencyRepo.findByUserId(user3Ctx.userId)
  if (!existingBalance_late3) {
    InMemoryCurrencyRepo._upsert({ userId: user3Ctx.userId, balance: 0, updatedAt: new Date() })
  }
  // 3人目の reply ログも事前挿入
  if (targetAuthorIdLate) {
    InMemoryIncentiveLogRepo._insert({
 ...
 */
When('そのスレッドに書き込みを1件行う', async function (this: BattleBoardWorld) {
 ...
 */
When('{string} がそのスレッドに書き込みを行う', async function (
  this: BattleBoardWorld,
  userName: string
) {
  assert(this.currentThreadId, 'スレッドが設定されていません')
  const ctx = await ensureNamedUser(this, userName)
  const existingBalance_revival = await InMemoryCurrencyRepo.findByUserId(ctx.userId)
 ...
 * See: features/phase1/incentive.feature @低活性スレッドに書き込み後30分以内に他ユーザーのレスが付くと
 */
When('30分以内に別のユーザーがそのスレッドにレスを書き込む', async function (
  this: BattleBoardWorld
) {
  assert(this.currentThreadId, 'スレッドが設定されていません')
  const otherCtx = await ensureNamedUser(this, 'UserRevivalFollower')
  const existingBalance_follower = await InMemoryCurrencyRepo.findByUserId(otherCtx.userId)
 ...
 */
When('30分以内に他のユーザーのレスが付かない', async function (this: BattleBoardWorld) {
 ...
 */
When(/^(\d+)日目の初回書き込みを行う$/, async function (
  this: BattleBoardWorld,
  _dayNumberStr: string
) {
 ...
 */
When('スレッドのレス番号 {int} に書き込みを行う', async function (
  this: BattleBoardWorld,
  targetPostNumber: number
) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
 ...
 * See: features/phase1/incentive.feature @同日4スレッド目の初参加ではボーナスは付与されない
 */
When('4つ目の未参加スレッドに書き込みを1件行う', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, 'ユーザーがログイン済みである必要があります')
  assert(this.currentEdgeToken, 'edge-token が設定されていません')
 ...
 * See: features/phase1/incentive.feature @その日の初回書き込みでログインボーナス +10 が付与される
 */
Then('書き込みログインボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  amount: number
) {
 ...
 * See: features/phase1/incentive.feature @同日の2回目以降の書き込みではボーナスは付与されない
 */
Then('書き込みログインボーナスは付与されない', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
  const logs = await findAllLogsForUser(this.currentUserId)
 ...
 * See: features/phase1/incentive.feature @その日の初回スレッド作成でボーナス +10 が付与される
 */
Then('スレッド作成ログインボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  amount: number
) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
  const logs = await findAllLogsForUser(this.currentUserId)
  const threadCreationLog = logs.find(l => l.eventType === 'thread_creation')
  assert(
 ...
 * See: features/phase1/incentive.feature @スレッド作成は書き込みログインボーナスの対象外である
 */
Then('スレッド作成ログインボーナス +{int} が付与される', async function (
  this: BattleBoardWorld,
  amount: number
) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
 ...
 * See: features/phase1/incentive.feature @同日の2回目以降のスレッド作成ではボーナスは付与されない
 */
Then('スレッド作成ログインボーナスは付与されない', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
  const logs = await findAllLogsForUser(this.currentUserId)
  const threadCreationLogs = logs.filter(l => l.eventType === 'thread_creation')
 ...
 * See: features/phase1/incentive.feature @スレッドにレスが10個付き、ユニークID 3個以上で +50 ボーナス
 */
Then('{string} にスレッド成長ボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  userName: string,
  amount: number
) {
 ...
 * See: features/phase1/incentive.feature @レスが10個付いてもユニークIDが3未満ならボーナスは付与されない
 */
Then('スレッド成長ボーナスは付与されない', async function (this: BattleBoardWorld) {
 ...
 * See: features/phase1/incentive.feature @スレッドにレスが10個付き、ユニークID 3個以上で +50 ボーナス
 */
Then('{string} の通貨残高が {int} になる', async function (
  this: BattleBoardWorld,
  userName: string,
  expected: number
) {
 ...
 * See: features/phase1/incentive.feature @レスが10個付いてもユニークIDが3未満ならボーナスは付与されない
 */
Then('{string} の通貨残高は {int} のまま変化しない', async function (
  this: BattleBoardWorld,
  userName: string,
  expected: number
) {
 ...
 */
Then('{string} に返信ボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  userName: string,
  amount: number
) {
 ...
 */
Then('{string} に返信ボーナスが合計 +{int} 付与される', async function (
  this: BattleBoardWorld,
  userName: string,
  totalAmount: number
) {
 ...
 * See: features/phase1/incentive.feature @60分以内に3人以上から返信が付くと +15 ボーナスが付与される
 */
Then('{string} にホットレスボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  userName: string,
  amount: number
) {
 ...
 * See: features/phase1/incentive.feature @返信が60分を超えた場合はホットレスボーナスは付与されない
 */
Then('ホットレスボーナスは付与されない', async function (this: BattleBoardWorld) {
  for (const [, ctx] of this.namedUsers) {
    const logs = await findAllLogsForUser(ctx.userId)
 ...
 */
Then('新スレッド参加ボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  amount: number
) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
 ...
 * See: features/phase1/incentive.feature @同一スレッドへの2回目の書き込みではボーナスは付与されない
 */
Then('新スレッド参加ボーナスは付与されない', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
  const logs = await findAllLogsForUser(this.currentUserId)
 ...
 * See: features/phase1/incentive.feature @低活性スレッドに書き込み後30分以内に他ユーザーのレスが付くと
 */
Then('{string} にスレッド復興ボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  userName: string,
  amount: number
) {
 ...
 * See: features/phase1/incentive.feature @30分以内に他ユーザーのレスが付かなければボーナスは付与されない
 */
Then('スレッド復興ボーナスは付与されない', async function (this: BattleBoardWorld) {
  for (const [, ctx] of this.namedUsers) {
    const logs = await findAllLogsForUser(ctx.userId)
 ...
 * See: features/phase1/incentive.feature @7日連続書き込みで +20 ストリークボーナスが付与される
 */
Then('書き込みログインボーナス +{int} に加え、ストリークボーナス +{int} が付与される', async function (
  this: BattleBoardWorld,
  loginAmount: number,
  streakAmount: number
) {
 ...
 * See: features/phase1/incentive.feature @途中で1日書き込みを休むとストリークがリセットされる
 */
Then('ストリークは1日目からリセットされる', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
  const user = await InMemoryUserRepo.findById(this.currentUserId)
  assert(user, 'ユーザーが見つかりません')
  assert.strictEqual(
 ...
 * See: features/phase1/incentive.feature @途中で1日書き込みを休むとストリークがリセットされる
 */
Then('ストリークボーナスは付与されない', async function (this: BattleBoardWorld) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
  const logs = await findAllLogsForUser(this.currentUserId)
 ...
 */
Then('キリ番ボーナスとして +{int} が付与される', async function (
  this: BattleBoardWorld,
  amount: number
) {
  assert(this.currentUserId, '通貨残高確認のためユーザーIDが必要です')
