...
import type { UserContext } from '../support/world'
import {
  InMemoryUserRepo,
  InMemoryThreadRepo,
  InMemoryPostRepo,
 ...
 */
Given('無料ユーザーがスレッド {string} を閲覧している', async function (
 ...
 */
Given('有料ユーザーがユーザーネーム {string} を設定済みである', async function (
  this: BattleBoardWorld,
  username: string
) {
  const AuthService = getAuthService()
 ...
 */
Given('スレッド {string} を閲覧している', async function (
  this: BattleBoardWorld,
  title: string
) {
 ...
 */
Given('ユーザーがスレッド {string} を閲覧している', async function (
  this: BattleBoardWorld,
  title: string
) {
  const AuthService = getAuthService()
...
When('本文を空にして書き込みボタンを押す', async function (this: BattleBoardWorld) {
...
Then('表示名は {string} である', async function (
  this: BattleBoardWorld,
...
Then('レスは追加されない', async function (this: BattleBoardWorld) {
  assert(this.currentThreadId, 'スレッドが設定されていません')
  const posts = await InMemoryPostRepo.findByThreadId(this.currentThreadId)
  assert.strictEqual(posts.length, 0, `レスが追加されないことを期待しましたが ${posts.length} 件存在します`)
})
 ...
 */
Given('ユーザー {string} とユーザー {string} がスレッド {string} を閲覧している', async function (
  this: BattleBoardWorld,
  userAName: string,
  userBName: string,
  title: string
  ...
  const { token: tokenA, userId: userIdA } = await AuthService.issueEdgeToken(ipHashA)
  ...
  const { token: tokenB, userId: userIdB } = await AuthService.issueEdgeToken(ipHashB)
...
When('{string} と {string} が同時に書き込みを行う', async function (
  this: BattleBoardWorld,
  userAName: string,
  userBName: string
) {
  ...
  concurrentPostResult = {
    userA: {
      ...
      postNumber: 'success' in resultA && (resultA as any).success ? (resultA as any).postNumber : undefined,
      error: 'error' in resultA ? (resultA as any).error : undefined,
    },
    userB: {
      success: 'success' in resultB && (resultB as any).success === true,
      postNumber: 'success' in resultB && (resultB as any).success ? (resultB as any).postNumber : undefined,
...
Then('両方のレスが正しくスレッドに追加される', async function (this: BattleBoardWorld) {
  assert(concurrentPostResult, '同時書き込み結果が存在しません')
  assert(concurrentPostResult.userA.success, `UserAの書き込みが失敗しました: ${concurrentPostResult.userA.error}`)
  assert(concurrentPostResult.userB.success, `UserBの書き込みが失敗しました: ${concurrentPostResult.userB.error}`)

  // 実際にスレッドに2件追加されていることを確認する
  assert(this.currentThreadId, 'スレッドが設定されていません')
  const posts = await InMemoryPostRepo.findByThreadId(this.currentThreadId)
  assert(posts.length >= 2, `スレッドに2件以上のレスが必要ですが ${posts.length} 件でした`)
})
