 ...
 */
When(/^ユーザーが \/auth\/verify で認証コードを送信する$/, async function (this: BattleBoardWorld) {
  ...
  if (result.success) {
    ...
    this.lastResult = { type: 'success', data: { verified: true, writeToken: result.writeToken } }
  } else {
 ...
 */
Given('ユーザーがedge-tokenを発行されているが認証コードを未入力である', async function (this: BattleBoardWorld) {
  const AuthService = getAuthService()
 ...
 * See: features/phase1/authentication.feature @edge-token発行後、認証コード未入力で再書き込みすると認証が再要求される
 */
Then('認証コード入力ページへの案内が再度表示される', function (this: BattleBoardWorld) {
  assert(this.lastResult, '操作結果が存在しません')
  assert.strictEqual(
    this.lastResult.type,
    'authRequired',
 ...
 */
When('ユーザーのIPアドレスが変わった後に書き込みを行う', async function (this: BattleBoardWorld) {
  assert(this.currentEdgeToken, 'ユーザーがログイン済みである必要があります')
...
// ---------------------------------------------------------------------------
// G3: edge-token 有効期限
// See: features/phase1/authentication.feature @edge-token Cookieの有効期限が切れると再認証が必要になる
// ---------------------------------------------------------------------------
 ...
 * See: features/phase1/authentication.feature @edge-token Cookieの有効期限が切れると再認証が必要になる
 */
When('edge-token Cookieの有効期限が切れた後に書き込みを行う', async function (this: BattleBoardWorld) {
  const PostService = getPostService()
...
Then('新しいedge-tokenが発行される', function (this: BattleBoardWorld) {
  assert(this.lastResult?.type === 'authRequired', `authRequired が返されることを期待しましたが "${this.lastResult?.type}" でした`)
  const token = this.lastResult.edgeToken
