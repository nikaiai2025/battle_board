...
import { InMemoryAuthCodeRepo, InMemoryUserRepo } from '../support/mock-installer'
import { InMemoryTurnstileClient } from '../support/mock-installer'
 ...
 */
Given('ユーザーが専ブラで未認証である', async function (this: BattleBoardWorld) {
  const AuthService = getAuthService()
 ...
 * See: features/constraints/specialist_browser_compat.feature @専ブラからの初回書き込みで認証案内が返される
 */
Then('レスポンスに認証コードと認証ページURLが含まれる', function (this: BattleBoardWorld) {
 ...
 */
Given('ユーザーが認証ページで認証を完了しwrite_tokenを取得している', async function (this: BattleBoardWorld) {
  const AuthService = getAuthService()
  ...
  const result = await AuthService.verifyAuthCode(code, 'dummy-turnstile-token', DEFAULT_IP_HASH)
 ...
 * See: features/constraints/specialist_browser_compat.feature @認証完了後にwrite_tokenをメール欄に貼り付けて書き込みが成功する
 */
When('bbs.cgiのメール欄に {string} を含めてPOSTする', async function (
  this: BattleBoardWorld,
  mailContent: string
) {
  const AuthService = getAuthService()
  ...
  if (isPlaceholder && !g4WriteToken) {
    this.lastResult = {
      type: 'error',
      message: 'g4WriteToken が設定されていません（Given ステップで取得してください）',
      code: 'INVALID_WRITE_TOKEN',
    }
    return
  }
  ...
  if ('success' in postResult && postResult.success) {
    this.lastResult = {
      type: 'success',
      data: { ...postResult, verifiedEdgeToken: edgeToken, emailSent: emailWithoutToken },
    }
...
Then('write_tokenが検証される', function (this: BattleBoardWorld) {
  assert(this.lastResult?.type === 'success', `write_token 検証成功（書き込み成功）を期待しましたが "${this.lastResult?.type}" でした`)
})
...
Then('edge-token Cookieが有効化される', function (this: BattleBoardWorld) {
  assert(this.lastResult?.type === 'success', `edge-token有効化（書き込み成功）を期待しましたが "${this.lastResult?.type}" でした`)
})
 ...
 * See: features/constraints/specialist_browser_compat.feature @認証完了後にwrite_tokenをメール欄に貼り付けて書き込みが成功する
 */
Then('メール欄のwrite_tokenは書き込みデータに含まれない', async function (this: BattleBoardWorld) {
 ...
 * See: features/constraints/specialist_browser_compat.feature @Cookie共有の専ブラでは認証後そのまま書き込みできる
 */
Given('ユーザーがWebブラウザで認証を完了している', async function (this: BattleBoardWorld) {
  const AuthService = getAuthService()
 ...
 * See: features/constraints/specialist_browser_compat.feature @Cookie共有の専ブラでは認証後そのまま書き込みできる
 */
Given('専ブラがWebブラウザとCookieを共有している', function (this: BattleBoardWorld) {
 ...
 * See: features/constraints/specialist_browser_compat.feature @無効なwrite_tokenでは書き込みが拒否される
 */
When('bbs.cgiのメール欄に無効なwrite_tokenを含めてPOSTする', async function (this: BattleBoardWorld) {
  const AuthService = getAuthService()
  ...
  if (!verifyResult.valid) {
    ...
    // 既存の "レスポンスのtitleタグに {string} が含まれる" ステップが lastBbsCgiHtml を参照するため設定する
    lastBbsCgiHtml = responseBuilder.buildError('無効なwrite_tokenでの書き込みは拒否されます')
  } else {
    // 検証が通ってしまった場合（テスト失敗）
