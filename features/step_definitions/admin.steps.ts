 ...
 */
Given('管理者でないユーザーがログイン済みである', function (this: BattleBoardWorld) {
 ...
 * スレッド "..." にレス >>5 が存在する。
 *
 * admin.feature の「管理者が指定したレスを削除する」シナリオで使用する。
 * thread.steps.ts の `スレッド {string} にレス >>1 が存在する` と競合しないよう
 * 固定リテラル >>5 を使用する。
 *
 * See: features/phase1/admin.feature @管理者が指定したレスを削除する
 * See: docs/architecture/bdd_test_strategy.md §4 ファイル分割方針
 */
Given('スレッド {string} にレス >>5 が存在する', async function (
...
When('レス >>5 の削除を実行する', async function (this: BattleBoardWorld) {
  ...
  assert(postId, 'レス >>5 の Post ID が見つかりません（Givenステップで作成が必要）')
...
When('レス >>999 の削除を実行する', async function (this: BattleBoardWorld) {
  ...
  this.lastResult = result.success
    ...
    : { type: 'error', message: `指定されたレスが見つかりません`, code: result.reason }
})
...
When('レス >>5 の削除を試みる', async function (this: BattleBoardWorld) {
  ...
  const postId = postNumberToId.get(5)
  if (!postId) {
    this.lastResult = { type: 'error', message: '指定されたレスが見つかりません', code: 'not_found' }
    return
  }
 ...
 */
When('スレッド {string} の削除を実行する', async function (
  this: BattleBoardWorld,
  threadTitle: string
) {
  assert(this.currentAdminId, '管理者がログイン済みである必要があります')
  assert(this.currentThreadId, `スレッド "${threadTitle}" の ID が設定されていません`)

  const AdminService = getAdminService()
  const result = await AdminService.deleteThread(this.currentThreadId, this.currentAdminId)
...
/**
 * レス >>5 の表示位置に "このレスは削除されました" と表示される。
 *
 * レスが is_deleted = true になっていることを確認する。
 ...
 */
Then('レス >>5 の表示位置に {string} と表示される', async function (
  this: BattleBoardWorld,
  ...
  const postId = postNumberToId.get(5)
  assert(postId, 'レス番号 5 の Post ID が見つかりません')
  ...
  assert.strictEqual(
    post.isDeleted,
    true,
    `レス >>5 の isDeleted が true であることを期待しましたが false でした`
  )
})

// ---------------------------------------------------------------------------
// Then: レス番号 >>N は欠番にならず保持される
// See: features/phase1/admin.feature @管理者が指定したレスを削除する
// ---------------------------------------------------------------------------
 ...
 * See: features/phase1/admin.feature @管理者が指定したレスを削除する
 */
Then('レス番号 >>5 は欠番にならず保持される', async function (this: BattleBoardWorld) {
  const postId = postNumberToId.get(5)
  assert(postId, 'レス番号 5 の Post ID が見つかりません')
  ...
  const post = await InMemoryPostRepo.findById(postId)
  assert(post !== null, 'レス >>5 のレコードが存在しません（欠番になってはいけない）')
  assert.strictEqual(
    post.postNumber,
    5,
    `レス番号 5 が保持されることを期待しましたが ${post.postNumber} でした`
  )
})
 ...
 * See: docs/architecture/bdd_test_strategy.md §4 ファイル分割方針
 */
Given('スレッド {string} が存在する', async function (
  this: BattleBoardWorld,
  threadTitle: string
) {
  const thread = await InMemoryThreadRepo.create({
 ...
 */
Then('スレッドとその中の全レスが削除される', async function (this: BattleBoardWorld) {
  assert(this.lastResult, '操作結果が存在しません')
  assert.strictEqual(
    this.lastResult.type,
    'success',
  ...
  assert(this.lastDeletedThreadId, '削除されたスレッド ID が設定されていません')
  ...
  const activePosts = await InMemoryPostRepo.findByThreadId(this.lastDeletedThreadId)
  assert.strictEqual(
    activePosts.length,
    0,
 ...
 */
Then('スレッド一覧から {string} が消える', async function (
  this: BattleBoardWorld,
  threadTitle: string
) {
