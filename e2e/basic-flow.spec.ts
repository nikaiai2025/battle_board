...
test.describe("Phase 1 基本フロー縦断テスト", () => {
	 ...
	 *
	 * テスト2（inlineSystemInfo = レス内マージ）と対になるテスト。
	 * こちらはコマンド結果が独立した★システムレス（>>3）として投稿されるケースを検証する。
	 *
	 * seedThreadWithPost() で別ユーザーの >>1 を事前投入し、
	 * `!abeshinzo` をコマンド付き書き込みとして投稿する。
	 * AbeshinzoHandler が eliminationNotice: "意味のないコマンドだよ" を返すと
	 * PostService が createPost() で★システム名義の独立レスを追加投稿する。
	 *
	 * 検証ポイント:
	 * - ユーザーの書き込み（>>2）が表示される
	 * - ★システム名義の独立レス（>>3）が表示される
	 * - >>3 の表示名に「★システム」が含まれる（PostItem.tsx L185-187: isSystemMessage=true）
	 * - >>3 の本文に「意味のないコマンドだよ」が含まれる
	 *
	 * See: features/command_system.feature @hidden_command
	 * See: src/lib/services/handlers/abeshinzo-handler.ts
	 * See: src/app/(web)/_components/PostItem.tsx L185-187
	 */
	test("隠しコマンド !abeshinzo で★システム名義の独立レスが投稿される", async ({
		...
		// ------------------------------------------------------------------
		// Step 3: !abeshinzo コマンドを書き込みフォームに入力して送信
		// 未認証のため AuthModal が表示されるので認証を完了する
		// ------------------------------------------------------------------
		...
		// ------------------------------------------------------------------
		// Step 5: ★システム名義の独立レス（>>3）が表示されることを確認
		// AbeshinzoHandler が eliminationNotice を返し、
		// PostService が createPost() で独立レスを追加投稿する
		// ------------------------------------------------------------------

		// >>3 が表示されること
		// See: src/app/(web)/_components/PostItem.tsx > #post-3
		await expect(page.locator("#post-3")).toBeVisible({
			timeout: 15_000,
		});

		// ------------------------------------------------------------------
		// Step 6: >>3 の表示名に「★システム」が含まれることを確認
		// PostItem.tsx L185-187: isSystemMessage=true の場合 "[システム] ★システム" と表示
		// ------------------------------------------------------------------
		await expect(page.locator("#post-3")).toContainText("★システム");

		// ------------------------------------------------------------------
		// Step 7: >>3 の本文に「意味のないコマンドだよ」が含まれることを確認
		// See: src/lib/services/handlers/abeshinzo-handler.ts > eliminationNotice
		// ------------------------------------------------------------------
