...
async function cleanupDatabase(request: APIRequestContext): Promise<void> {
	...
	const threadsRes = await request.delete(
		`${supabaseUrl}/rest/v1/threads?id=neq.00000000-0000-0000-0000-000000000000`,
		{ headers },
	);
	if (!threadsRes.ok()) {
		throw new Error(
			`cleanupDatabase: threads DELETE failed (status: ${threadsRes.status()})`,
		);
	}

	const edgeTokensRes = await request.delete(
		`${supabaseUrl}/rest/v1/edge_tokens?id=neq.00000000-0000-0000-0000-000000000000`,
		{ headers },
	);
	if (!edgeTokensRes.ok()) {
		throw new Error(
			`cleanupDatabase: edge_tokens DELETE failed (status: ${edgeTokensRes.status()})`,
		);
	}
}
