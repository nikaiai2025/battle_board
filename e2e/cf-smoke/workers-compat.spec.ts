 ...
 *
 * 検証対象:
 *   1. GET /livebot/subject.txt  — Shift_JIS レスポンス（iconv-lite）
 *   2. GET /livebot/dat/{key}.dat — rewrite ルール動作
 *   3. GET /bbsmenu.html              — Shift_JIS レスポンス（iconv-lite）
 *   4. GET /bbsmenu.json              — JSON API 基本動作
 *   5. GET /livebot/SETTING.TXT   — Shift_JIS レスポンス（iconv-lite）
...
test.describe("CF Smoke: Workers Runtime 互換性", () => {
	 ...
	 */
	test("GET /livebot/subject.txt -- Shift_JIS Content-Type で応答する（500 でない）", async ({
		request,
	}) => {
	 ...
	 * See: docs/architecture/bdd_test_strategy.md S13.2
	 */
	test("GET /livebot/dat/{key}.dat -- rewrite が動作する（500 でない）", async ({
		request,
	}) => {
		const response = await request.get(`/${BOARD_ID}/dat/9999999999.dat`);
	 ...
	 * See: docs/architecture/bdd_test_strategy.md S13.2
	 */
	test("GET /livebot/SETTING.TXT -- 200 + Shift_JIS レスポンス", async ({
		request,
	}) => {
