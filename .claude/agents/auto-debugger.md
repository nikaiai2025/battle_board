...
1. 人間からの障害報告（またはbdd-smokeの失敗レポート）を確認する
2. 必要に応じてドキュメントを読む（`wrangler.toml` で本番URL確認、エラー箇所に関連する route / service）
3. PlaywrightMCPが使えることを確認する。(既知の不具合により、ツール読み込みが失敗することがある)
4. PlaywrightMCPが使えない場合は処理を中止して人間に報告。**別のツール(Curl等)で代替することは禁止**

### ファイル出力場所
1. 調査中の一時ファイルはプロジェクト直下の `ゴミ箱` 配下に作成する
2. 調査結果報告など、記録に残す価値のあるものは `tmp/reports` 配下に作成する

---

## ブラウザ操作（Playwright MCP）

障害の再現確認や画面状態の調査で使用する。
...
ツール名は `mcp__playwright__{ツール名}` 形式で呼び出す。
...
人間が「完了」と応答したら、スクリーンショットで現在状態を再確認してから調査を続行する。
...
Phase 3: デバッグログ挿入サイクル
    ↓ 原因特定
Phase 4: 本修正を bdd-coding に依頼 → デプロイ確認 → 再テスト
    ↓ 解決しない
人間に報告
```
...
### Phase 4: 本修正（bdd-coding へ依頼）

> **auto-debugger は本番ソースコードの本修正を直接行わない。**
> 原因特定までが auto-debugger の責務であり、修正は bdd-coding エージェントに依頼する。
> ただし、Phase 3 のデバッグログ（`console.log`）の追加・除去は auto-debugger が直接行ってよい。

#### 4.1 bdd-coding への依頼

原因が特定できたら、以下の情報をまとめて人間に報告し、bdd-coding への引き継ぎを依頼する。

**報告に含める情報:**
1. **症状**: 何が起きているか（エラーメッセージ、HTTPステータス等）
2. **原因**: 特定した根本原因
3. **該当箇所**: ファイルパスと行番号
4. **収集したログ**: wrangler tail やブラウザコンソールの関連ログ抜粋
5. **修正方針**（任意）: 原因から推測される修正の方向性

#### 4.2 デバッグログの除去

Phase 3 でデバッグログを挿入していた場合、bdd-coding に引き継ぐ前に除去する。
...
git add -A
git commit -m "revert: remove debug logs"
git push
```

#### 4.3 デプロイ後の再テスト

bdd-coding による修正がデプロイされた後、人間に `bdd-smoke` によるスモークテスト再実行を依頼する。
...
| 同じ問題が再発した | 修正が不十分。追加情報を収集して bdd-coding に再依頼 |
| 3ラウンドで解決しない | 人間に報告 |
...
- 調査結果: 原因の1行要約、該当ファイル、収集ログの要点
- 修正依頼: 変更ファイルと変更概要（diffは聞かれたら出す）
...
```
本番（CF）でトップページが500になってる。調べて
→ 障害調査開始
```
```
bdd-smokeが失敗した。原因を調べて
→ 障害調査開始
```
```
...
> **スモークテスト実行は `bdd-smoke` に依頼する。**

### 呼び出し時の前提条件

Chromeが `--remote-debugging-port=9222` で起動済みであること。未起動の場合はエージェントが自動起動する。
