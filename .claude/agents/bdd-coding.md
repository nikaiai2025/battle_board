...
2. 割り当てられたタスク指示書 `tmp/tasks/task_{TASK_ID}.md` を読む
3. タスク指示書の `locked_files` を確認し、変更対象ファイルを把握する
4. タスク指示書の `artifacts_dir` が指定されている場合、成果物出力ディレクトリのパスを把握する
5. タスク指示書に記載された必読ドキュメント（featureファイル、設計書等）を順に読む
6. タスク指示書に `## 作業ログ` セクションが既に存在し、チェックポイントに中断記録がある場合、エスカレーション解決後の再開と判断する。チェックポイントの内容と `escalation_resolution` セクションを確認し、中断地点から作業を継続する
7. タスクステータスを `in_progress` に更新する
