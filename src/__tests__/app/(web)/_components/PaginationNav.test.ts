...
describe("generatePaginationLinks: 250件のスレッド", () => {
	...
	const links = generatePaginationLinks("livebot", "1234567890", 250);
	...
	it('"最新100" のリンクが含まれる', () => {
		...
		expect(found).toBeDefined();
		expect(found?.href).toBe("/livebot/1234567890/l100");
	});
	...
	it('"全件" のリンクが含まれる', () => {
		...
		expect(found).toBeDefined();
		expect(found?.href).toBe("/livebot/1234567890/1-250");
	});
...
describe("generatePaginationLinks: 100件のスレッド", () => {
	...
	const links = generatePaginationLinks("livebot", "9999999999", 100);

	it('"1-100" のレンジリンクが含まれる', () => {
		const found = links.find((l) => l.label === "1-100");
		expect(found).toBeDefined();
		expect(found?.href).toBe("/livebot/9999999999/1-100");
	});
...
describe("generatePaginationLinks: 101件のスレッド", () => {
	...
	const links = generatePaginationLinks("livebot", "1111111111", 101);
	...
	it('"101-101" の最終レンジリンクが含まれる', () => {
		...
		expect(found).toBeDefined();
		expect(found?.href).toBe("/livebot/1111111111/101-101");
	});
});
...
describe("generatePaginationLinks: 200件のスレッド", () => {
	...
	const links = generatePaginationLinks("livebot", "2222222222", 200);
...
describe("generatePaginationLinks: エッジケース", () => {
	it("postCount が 51 の場合（最小表示ケース）", () => {
		...
		const links = generatePaginationLinks("livebot", "1234567890", 51);
	...
	it("postCount が 1000 件の大量データ（1万件超えではないが境界確認）", () => {
		const links = generatePaginationLinks("livebot", "1234567890", 1000);
	...
	it("postCount が 10001 件の大量データ", () => {
		...
		const links = generatePaginationLinks("livebot", "1234567890", 10001);
		const rangeLinks = links.filter(
			(l) => l.type === "range" && l.label !== "全件",
		);
