 ...
 *
 * 7200〜14400分（120〜240時間）のランダムな整数（分単位）を返す。
...
/** デフォルトの最小投稿間隔（分）*/
const DEFAULT_MIN_MINUTES = 7200;

/** デフォルトの最大投稿間隔（分）*/
const DEFAULT_MAX_MINUTES = 14400;

/**
 * TopicDrivenSchedulingStrategy クラス。
 *
 * コンストラクタで min/max を指定可能（デフォルト: 7200〜14400分、120〜240時間）。
...
export class TopicDrivenSchedulingStrategy implements SchedulingStrategy {
	/**
	 * @param minMinutes - 最小投稿間隔（分、デフォルト: 7200）
	 * @param maxMinutes - 最大投稿間隔（分、デフォルト: 14400）
	 */
	constructor(
