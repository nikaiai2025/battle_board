 ...
 *
 * 600〜1200分（10〜20時間）のランダムな整数（分単位）を返す。
...
/** デフォルトの最大書き込み間隔（分）*/
const DEFAULT_MAX_MINUTES = 1200;

/**
 * FixedIntervalSchedulingStrategy クラス。
 *
 * コンストラクタで min/max を指定可能（デフォルト: 600〜1200分（10〜20時間））。
...
export class FixedIntervalSchedulingStrategy implements SchedulingStrategy {
	/**
	 * @param minMinutes - 最小書き込み間隔（分、デフォルト: 600）
	 * @param maxMinutes - 最大書き込み間隔（分、デフォルト: 1200）
	 */
	constructor(
