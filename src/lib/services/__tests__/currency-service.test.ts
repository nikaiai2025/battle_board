	...
	describe("credit", () => {
		describe("正常系: 通常の加算", () => {
			...
			it("大きな金額（10000）の加算も正常に処理される", async () => {
				...
				expect(CurrencyRepository.credit).toHaveBeenCalledWith(
					"user-001",
					10000,
				);
			});
	...
	describe("deduct", () => {
		...
		describe("異常系: DB障害", () => {
			it("CurrencyRepository.deduct がエラーをスローした場合は伝播する", async () => {
				...
				await expect(deduct("user-001", 10, "command_tell")).rejects.toThrow(
					"CurrencyRepository.deduct failed",
				);
			});
		});
	});
