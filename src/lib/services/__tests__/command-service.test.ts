 ...
 *
 * カバレッジ対象（task_TASK-068.md §完了条件より）:
 *   - 正常なコマンド実行（!w）→ 成功結果
 *   - 通貨不足 → エラー結果（通貨未消費）
...
import type { ICurrencyService, CommandExecutionInput } from '../command-service'
...
function createMockCurrencyService(
  ...
  const deductMock = vi.fn().mockImplementation(
...
describe('CommandService', () => {
  ...
  describe('初期化', () => {
    it('commands.yaml を正常に読み込み、Registryを構築できる', () => {
      ...
      expect(() => {
        new CommandService(currencyService)
      }).not.toThrow()
    })

    it('getRegisteredCommandNames が登録済みコマンド名を返す', () => {
      const currencyService = createMockCurrencyService()
      const service = new CommandService(currencyService)
...
`
  ...
  describe('!w コマンド（無料コマンド）', () => {
    ...
    it('本文中の任意の位置に !w が含まれていても実行される', async () => {
      ...
      const result = await service.executeCommand(createInput('なんか笑えるわ !w >>3 ほんとに'))
  ...
  describe('!tell コマンド（有料コマンド）', () => {
    ...
    it(
      ...
      '通貨不足の場合はコマンドが実行されずエラーメッセージが返される',
      async () => {
    ...
    it('楽観的ロックにより deduct が失敗した場合もエラーメッセージが返される', async () => {
      ...
      const currencyService: ICurrencyService = {
        ...
        deduct: vi.fn().mockResolvedValue({ success: false, reason: 'insufficient_balance' }),
      }
      const service = new CommandService(currencyService)
    ...
    it('!tell stub は未実装メッセージを返す（次タスクで本実装予定）', async () => {
      const currencyService = createMockCurrencyService(100)
      const service = new CommandService(currencyService)

      const result = await service.executeCommand(createInput('!tell >>5'))

      // stub なので success=false かつ systemMessage に「実装中」が含まれる
      // currencyCost は stub 実行後に 0 になる（success=false のため）
      expect(result).not.toBeNull()
      expect(result!.systemMessage).toContain('実装中')
    })
  })
  ...
  describe('未登録コマンド', () => {
    it(
      ...
      '存在しないコマンドの場合は null が返される',
      async () => {
        const currencyService = createMockCurrencyService()
        const service = new CommandService(currencyService)

        const result = await service.executeCommand(createInput('!unknowncommand なんか適当に'))
    ...
    it('コマンドを含まない通常テキストの場合は null が返される', async () => {
      ...
      const result = await service.executeCommand(createInput('普通の書き込みです'))
