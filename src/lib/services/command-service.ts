...
export class CommandService {
   ...
   */
  async executeCommand(input: CommandExecutionInput): Promise<CommandExecutionResult | null> {
...
// ---------------------------------------------------------------------------
// !tell ハンドラ stub（AccusationService 依存があるため次タスクで実装）
// ---------------------------------------------------------------------------

/**
 * !tell ハンドラの stub 実装。
 * AccusationService への依存があるため、本実装は次タスク（TASK-069以降）で行う。
 * Registry の整合性を保つために stub として登録する。
 *
 * See: task_TASK-068.md §スコープ外
 * See: docs/architecture/components/command.md §3.1 依存先 > AccusationService
 */
class TellHandlerStub implements CommandHandler {
  ...
  async execute(_ctx: CommandContext): Promise<CommandHandlerResult> {
    ...
    return {
      success: false,
      systemMessage: '!tell コマンドは現在実装中です',
    }
  }
}
