 ...
 *   - 認証必要: titleタグに "ＥＲＲＯＲ"（全角）+ 認証ページURL + 手順説明
 *
...
${escapedMessage}
	 ...
	 *
	 * 専ブラ向けに認証ページURL・手順説明を明示する。
	 * 専ブラは WebView を持たないため、ユーザーはブラウザで認証を完了後、
	 * Cookie 共有の専ブラではそのまま書き込み可能。
	 * Cookie 非共有の場合は write_token をメール欄に貼り付けて再書き込みする（G4対応）。
	 *
	 * 認証ページURL形式（絶対URL）: {baseUrl}/auth/verify?token={edgeToken}
	...
	buildAuthRequired(edgeToken: string, baseUrl: string): string {
		...
		const authUrl = `${normalizedBaseUrl}/auth/verify?token=${escapedToken}`;
		return `<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=Shift_JIS">
...
<body>
<b>ＥＲＲＯＲ</b><br>
専ブラの初回書き込み前には認証が必要です。<br>
<br>
【手順】<br>
以下のURLにブラウザでアクセスして認証を完了してください<br>
<br>
【認証URL】<br>
<a href="${authUrl}">${authUrl}</a><br>
<br>
【過去にメールアドレス登録済の方へ（端末変更の引継ぎ等）】<br>
ブラウザでマイページにログインし、PAT（#pat_a1b2c3...の形式の文字列）をコピーして、専ブラのメールアドレス欄に入力してください。<br>
（メールアドレス登録していない場合は引き継ぎ不可）
</body>
</html>`;
	}
