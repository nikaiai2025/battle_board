...
**Sprint-53 完了 — PostListLiveWrapper二重表示バグ修正**

`router.refresh()`後にSSR側の新レスとClient Component側のnewPostsが重複表示されるバグを修正。
useEffectで`initialLastPostNumber` propの変化を検知し、SSRカバー済みレスをnewPostsから除去する。
...
- 横展開未了: bot-service.ts / fixed-message.ts の同パターン修正（次スプリント以降）
...
- vitest: 48ファイル / 1201テスト / 全PASS（Sprint-53で+10件: PostListLiveWrapper二重表示バグ修正テスト追加）
