...
| Step 5 | 書き込み + スレッド管理 | **次スプリント候補** | — |
| Step 6 | インセンティブサービス | 未着手（Step 5 に依存） | — |
| Step 7 | Web UI | 未着手（Step 5,6 に依存） | — |
| Step 8 | 管理機能 | 未着手（Step 5 に依存） | — |
| Step 9 | 専ブラ互換 Adapter | 未着手（Step 5 に依存） | — |
| Step 10 | マイページ + 仕上げ | 未着手（Step 5,4 に依存） | — |

## テスト状況

- vitest: 5ファイル / 208テスト / 全PASS

## 次スプリント候補

- **Step 5（書き込み + スレッド管理）** — Step 3, 4 完了により着手可能。最初の垂直スライス。
- Step 5 は CurrencyService + PostService + API Routes を含む大きなステップ
