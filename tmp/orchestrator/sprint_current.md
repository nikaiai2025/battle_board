...
**Sprint-95 完了 — BOT投稿FK制約違反修正 + 固定案内板リンクフルURL化**

### Sprint-95の成果
- TASK-267: 固定案内板リンクをフルURL化（/mypage, /dev/ → フルURL。専ブラ自動リンク対応）
- TASK-268: BOT投稿FK制約違反修正（author_id=NULL維持 + コマンドパイプラインuserId分離。単体テスト3件追加）
- 人間変更同梱: 開発連絡板レトロUI更新（marquee等）、レトロWebリサーチ3件、UIチューニングエージェント追加
...
- vitest: 82ファイル / 1678テスト / 全PASS
- cucumber-js: 301シナリオ / 285 passed / 0 failed / 16 pending
...
- **本番スモークテスト (Sprint-95後):** 30/35 PASS（5件skip: ローカル限定）
...
| ③ | !aori | `command_aori.feature` | BOT召喚 + 非同期キュー | **ブロック中**（BOT本番動作検証待ち） |
| ④ | !newspaper | `command_newspaper.feature` | AI API (Gemini) + 非同期 | **ブロック中**（③に依存 + BOT本番動作検証待ち） |

**ブロッカー**: cron 500エラーの2つの原因を修正済み（Sprint-93: IncentiveServiceスキップ、Sprint-95: FK制約違反修正）。デプロイ済みだがBOTが未稼働のため本番検証が未完了。次回BOT cron実行で正常動作を確認後、③④のブロック解除。
...
- **[修正済み・本番検証待ち] cron 500エラー**: 原因2つ修正済み — (1) Sprint-93 TASK-263: IncentiveServiceスキップ (2) Sprint-95 TASK-268: FK制約違反修正。両方デプロイ済み。次回BOT cron実行で検証。練習コマンド③④のブロッカー解除は本番検証後
