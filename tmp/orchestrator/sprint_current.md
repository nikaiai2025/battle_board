...
**Sprint-48 完了 — 固定スレッド自動デプロイ**

scriptsディレクトリの「完成済みだが本番未実行」問題を解消。GitHub Actionsで自動化。

### Sprint-48の成果
- 固定スレッド自動upsert: GitHub Actionsで `config/commands.yaml` 変更時に自動実行 + `workflow_dispatch` で初回投入可能
- GitHub Secrets登録済み: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF`
...
- vitest: 45ファイル / 1141テスト / 全PASS（Sprint-45比 +3件 — schema-consistency.test.ts新規追加）
