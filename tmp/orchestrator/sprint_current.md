...
### Sprint-49の成果
- 前方引数対応: `>>N !cmd` を `!cmd >>N` と等価に認識
- 後方優先: `>>3 !tell >>5` → 後方の `>>5` が対象
- 全角スペース対応: 前方・後方とも半角/全角スペースを区切りとして許容
- 非認識条件: アンカーとコマンドの間にテキストや改行がある場合は前方引数として認識しない
- 既存undefined 2件も解消（BDDステップ定義追加による）
...
- vitest: 45ファイル / 全PASS（command-parser.test.ts +11件追加）
