...
**Sprint-74 完了 — E2Eスモークテスト全ページカバー + バグ3件修正**

E2Eナビゲーションテスト（Phase A）の未カバー8ページを追加し、全13ページのカバレッジを達成。作業中に発見した3件のバグ（admin認証RLS汚染、E2E cleanup、mypage API認証方式不整合）も修正。
...
- 残: ai_accusation.steps.ts の botRepository 注入修正（前スプリントから継続）
...
- vitest: 66ファイル / 1412テスト / 全PASS（Sprint-74で+17件）
- cucumber-js: 256シナリオ (240 passed, 16 pending) / 0 failed
...
本番環境での管理者アカウント登録、およびスモークテスト用環境変数ファイル `.env.prod.smoke`（`PROD_SMOKE_EDGE_TOKEN`, `PROD_ADMIN_EMAIL`, `PROD_ADMIN_PASSWORD`）の設定が完了。
次回スモークテスト実行時に認証必須テスト（17件）が通る見込み。
