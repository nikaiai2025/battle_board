...
**Phase 2 準備（認証フロー是正完了 → デプロイ待ち）**

Sprint-17で認証フロー是正（G1〜G4）が完了。デプロイ後にChMate実機検証（G5）を実施予定。
...
**主な変更:**
- `users.is_verified` フラグ追加: edge-tokenの認証完了状態を管理
- `verifyEdgeToken` に未検証チェック追加: 認証コード未入力での書き込みを防止（G1）
- `write_token` 方式導入: 専ブラがTurnstile不可のため、Webブラウザ認証→mail欄トークンで橋渡し（G4）
- `/auth/verify` ページ新規作成: Web UI・専ブラ共用の認証ページ
- IP変更時の継続性（G2）、edge-token有効期限切れ（G3）のBDDシナリオ明文化

**テスト結果:**
- vitest: 18ファイル / 552テスト / 全PASS
