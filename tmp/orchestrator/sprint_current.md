...
**Sprint-72 完了 — BOT total_posts インクリメント漏れ修正**

BOTヘルスチェック（C8）で `bots.total_posts = 0`（実際は4件投稿済み）を検出。`BotService.executeBotPost()` で `incrementTotalPosts` の呼び出しが欠落していた実装忘れ。撃破時の戦歴表示が常に0件となる問題を修正。
...
- vitest: 65ファイル / 1388テスト / 全PASS（Sprint-72で+2件）
