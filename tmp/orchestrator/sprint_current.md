...
**Sprint-70 完了 — BDDステップスタブ残存バグ修正 + 本登録ページ実装**

本番手動テストで発見されたインシデント2件（!attack撃破時の★システム独立レス未投稿、ボット既存スレッド検証の空検証）を修正。加えて、E2Eテスト追加により検出された本登録ページ（/register/email, /register/discord）の404を解消。
