...
**Sprint-73 完了 — ログアウトボタン追加 + accused_count インクリメント漏れ修正**

D-06 mypage.yaml 更新に対応しマイページにログアウトボタンを追加。また、BOT告発時の `accused_count` インクリメント漏れ（total_postsと同種のバグ）を修正し、LL-010（派生カウンタ保護）を教訓記録に追記。
...
- vitest: 65ファイル / 1395テスト / 全PASS（Sprint-73で+7件）
