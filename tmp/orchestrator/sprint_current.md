...
**Sprint-88 完了 — タイムゾーンバグ修正（全日時表示をJST固定に統一）**
...
- vitest: 78ファイル / 1635テスト / 全PASS
- cucumber-js: 290シナリオ / 274 passed / 0 failed / 16 pending
...
- **本番スモークテスト (Sprint-88後):** 29/34 PASS（5件skip: ローカル限定テスト）
...
- **cron 500エラー**: `wrangler tail` で `/api/internal/bot/execute` が500を返していることを確認（2026-03-21）。Workers Observability Logsを有効化済み。ログが溜まった頃に `auto-debugger` で調査する
