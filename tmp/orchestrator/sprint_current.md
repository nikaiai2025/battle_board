...
| ChMate | Cloudflare | ✅ | ❌ | 読み取り可。書き込み時の認証フロー後に書き込み不能（後述） |

### ChMate書き込み問題（要調査）

- **症状**: 書き込み→認証画面表示（正常）→キャンセル→以降書き込み不能
- **想定正常動作**: 認証キャンセル後も再度書き込み試行可能であるべき
- **Sikiとの差異**: Sikiでは同等の操作で問題なし
- **疑い**: bbs.cgiの認証レスポンス（`buildAuthRequired`）がChMate固有のパース挙動と噛み合っていない可能性。認証キャンセル後にChMateがedge-token Cookieの状態やレスポンスをキャッシュしている可能性もある
- **優先度**: 中（ChMateユーザーの書き込みに影響。ただしSikiは正常）

## Phase 2 着手前の課題

`tmp/phase2_prerequisites.md` に整理済み。進捗:
1. ~~専ブラ互換の実機テスト~~ → 上記テーブル参照。**ChMate書き込み問題が残存**
...
- vitest: 15ファイル / 476テスト / 全PASS
- cucumber-js: 88シナリオ / 424ステップ / 全PASS（除外3件: Phase 2依存）
