...
**Sprint-84 完了 — ウェルカムシーケンス + CF Cron + マイページ 実装 Wave 1-2**

### Sprint-84の成果（実装 Wave 1-2）
- TASK-238: CF Cron scheduled ハンドラ（self-fetch方式）+ wrangler.toml + bot-scheduler.yml無効化
- TASK-239: ウェルカムシーケンス同期部分（PostService Step 6.5/11.5）+ Currency v5 (INITIAL_BALANCE=0) + pending_tutorials DB
- TASK-240: チュートリアルBOT 3 Strategy + bot_profiles.yaml tutorial + PostInput.botUserId + 日次リセット除外
- TASK-241: Mypage searchByAuthorId + getPostHistory拡張 + APIルート params + OpenAPI更新
...
- vitest: 78ファイル / 1628テスト / 全PASS
- cucumber-js: 282シナリオ（welcome.feature 11 + mypage.feature 8 追加）
  - 既存255 passed維持（BDD後方互換修正済み）
  - 新規19シナリオのstep definitionsは Sprint-85 で実装予定
...
- **本番スモークテスト (Sprint-81後):** 30/35 PASS（5件skip: ローカル限定テスト）
...
- **cron移行**: CF Cron Triggers 導入済み（Sprint-84 TASK-238）。荒らし役BOT移行完了。チュートリアルBOTスポーン処理は Sprint-85 で実装予定
