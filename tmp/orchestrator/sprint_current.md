...
**Sprint-113 完了 — BOT情報漏洩修正(LEAK-1/2/3) + 専ブラedgeTokenフォールバック**

### Sprint-113の成果
- TASK-307: LEAK-1修正 — `!w` でBOT草カウント「計0本」固定を解消（migration + handler + repo + tests）
- TASK-308: LEAK-2/3修正 — `!hissi`/`!kinou` でBOT書き込みエラー解消（handler×2 + repo + DI + tests）
- TASK-309: Bot interface grassCount追加に伴う型エラー修正（bot-service + テスト7ファイル）
- 専ブラ向けedgeTokenボディフォールバック（auth/verify route + tests、人間実装）
- BDDシナリオ v4/v2: reactions.feature +1, investigation.feature +2（人間承認済み）
- vitest 1782 PASS / cucumber-js 344シナリオ（322 passed, 6 failed, 16 pending）
- コミット: 300e04d
- 本番スモーク: **29/34 PASS**（5件は設計上のスキップ）

### 既知のテスト失敗（Sprint-113以前から存在、本スプリント未変更）
- vitest 4 failed: registration-service.test.ts loginWithEmail（SUPABASE_URL環境変数未設定）
- BDD 6 failed: user_registration.feature ×4（同上） + theme.feature ×2（フォントロック/リセット）
...
- vitest: 1782 PASS / 4 failed（registration-service loginWithEmail環境依存）
- cucumber-js: 344シナリオ / 322 passed / 6 failed / 16 pending
  - 6 failed: user_registration ×4（loginWithEmail環境依存） + theme ×2（フォントロック/リセット）
...
- **本番スモークテスト (Sprint-113後):** 29/34 PASS（5件は設計上のスキップ）
...
| ID | 内容 | 優先度 | 推奨時期 |
|---|---|---|---|
| TD-TEST-001 | registration-service loginWithEmail テストのモック不足（BDD 4 + vitest 4） | 高 | 次スプリント |
| TD-TEST-002 | theme.feature フォントロック/リセット BDD 2件FAIL | 中 | 次スプリント |
