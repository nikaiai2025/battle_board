...
**Phase 2 準備（ChMate認証問題 解決済み → 新規BDDシナリオ実装中）**

ChMate毎回認証問題は**解決済み**。根本原因はSet-Cookieの`Secure`/`SameSite`属性がChMateのHTTPクライアントと非互換であったこと。eddist準拠で両属性を除去し、Max-Ageを365日に延長。人間による実機検証で正常動作を確認。

Sprint-20で追加された7つの新規BDDシナリオ（Cookie互換・絵文字処理・URL体系・HTTP:80制約）の実装がSprint-21の対象。

## Sprint-20 サマリー（ChMate認証問題 — 根本原因特定・解決）

診断ログで根本原因を特定し、修正・実機検証まで完了。

**根本原因:** Set-Cookieの`Secure`属性。ChMate等の専ブラはSecure属性付きCookieを保存しない。
**修正:** eddist参考実装に倣い、Secure/SameSite属性を除去。Max-Age=365日に延長。
**検証:** 3回の実機テスト（SameSite=Lax → SameSite=None;Secure → 両属性除去）で問題解消を確認。

**主な変更:**
- Set-CookieからSecure/SameSite属性を除去（eddist準拠）
- Max-Age: 30日→365日（eddist準拠）
- bbs.cgi route handlerに6箇所の診断ログ追加
- 通常の成功レスポンスにもSet-Cookie: edge-tokenを追加（eddist整合）
- featureファイルに7シナリオ追加（Cookie互換・絵文字処理・URL体系・HTTP:80制約）
- テストレポート作成（`tmp/test_report_sprint20.md`）

**教訓:** 専ブラ向けSet-Cookieには`Secure`/`SameSite`を設定してはならない。eddistの実装が専ブラ互換性の権威源。

## Sprint-19 サマリー（ChMate毎回認証問題修正 — 差し戻し含む）

ChMate実機テストで発覚した「毎回認証コードが必要」問題に対応。

**経緯:**
1. TASK-052: write_token永続化を実装 → **人間が却下**（回避策。根本原因に向き合うべき）→ **revert済み**
2. TASK-053: /auth/verifyコピーボタン追加 → **採用（維持）**
3. TASK-054: アーキテクト分析（Cookie永続化の正攻法）→ titleパターンが原因と結論
4. TASK-055: buildAuthRequired titleをＥＲＲＯＲに変更 → **デプロイ済みだが「直ってない」と報告**

**現在の状態:**
- write_token仕様: ワンタイム・10分有効（ESC-AUTH-REVIEW-1承認済みの元仕様に復元）
- buildAuthRequired title: `<title>ＥＲＲＯＲ</title>`（eddist整合）
- /auth/verifyコピーボタン: あり
...
- **Sprint-19**: write_token永続化を試みるも人間が却下→revert。titleをＥＲＲＯＲに変更するも効果なし
- **Sprint-20**: **根本原因特定・解決**。Secure/SameSite属性除去 + Max-Age=365日 + 成功レスポンスへのSet-Cookie追加。ChMate実機検証で正常動作確認

## Phase 2 着手前の課題

`tmp/phase2_prerequisites.md` に整理済み。進捗:
1. ~~専ブラ互換の実機テスト~~ → **完了**（ChMate/Siki共に正常動作）
...
15. ~~本番DBマイグレーション適用（00005_auth_verification.sql）~~ → **適用済み**（Sprint-18〜20のChMate実機テストでis_verified/write_tokenが正常動作していたことが証拠）

## スプリント履歴

| Sprint | 対応Step | ステータス | 計画書 |
|---|---|---|---|
| Sprint-22 | Web UI機能整備（マイページ導線 + 認証整合） | completed | `tmp/orchestrator/sprint_22_plan.md` |
| Sprint-21 | 新規BDDシナリオ実装（絵文字・URL互換・Cookie互換）+ ログ除去 | completed | `tmp/orchestrator/sprint_21_plan.md` |
| Sprint-20 | ChMate認証問題 — 根本原因特定・解決 | completed | `tmp/orchestrator/sprint_20_plan.md` |
| Sprint-19 | ChMate毎回認証問題修正（write_token永続化→却下→title変更） | completed | `tmp/orchestrator/sprint_19_plan.md` |
