...
**Sprint-58 完了 — BOT本番稼働ブロッカー解消（seedデータCI化 + createBotServiceバグ修正）**

BOT本番稼働を阻害していた2つのバグを修正。(1) botsテーブルにレコードがない問題をseedマイグレーション（CI自動適用）で解決。(2) createBotService()のcreatePostFn未注入バグを修正。

### Sprint-58の成果
- TASK-161: 00016_seed_arashi_bot.sql（冪等INSERT）+ createBotService() threadRepository/createPostFn注入修正
- テスト: bot-service.test.ts 41件PASS、全体1269件PASS（schema-consistency除く）
- BOT稼働ブロッカー: 全解消。次の :00 or :30 cron起動で初回書き込みが発生する見込み
