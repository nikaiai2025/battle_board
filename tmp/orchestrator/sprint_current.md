...
**Sprint-77 完了 — 画像URLサムネイル表示(@image_preview)実装**

### Sprint-77の成果
- TASK-212: @image_preview設計（bdd-architect）— URL検出ロジック・コンポーネント構成・セキュリティ方針
- TASK-213: @image_preview実装（bdd-coding）— url-detector.ts, ImageThumbnail.tsx, PostItem.tsx parsePostBody追加, BDDステップ定義, 単体テスト46件
- TASK-214: kinou-handler TZ不具合修正 — getYesterdayJst→getYesterdayUtc（UTC/JSTの日付不整合解消）
- テスト: vitest 1527件全PASS / cucumber-js 271シナリオ(255 passed, 16 pending, 0 failed)
- 本番スモーク: 23/24 PASS (1 skip: ローカル限定)
- コミット: 78138b0

### 手動修正コミット (1b66d0f) — 2026-03-21
- attacks.post_id nullable化 + CommandContext.dailyId追加（本番障害修正）
- features/thread.feature @image_preview 4シナリオ追加、ドキュメント整備
- コミット: 1b66d0f
...
- vitest: 71ファイル / 1527テスト / 全PASS（Sprint-77で+46件: url-detector）
- cucumber-js: 271シナリオ (255 passed, 16 pending) / 0 failed
  - Sprint-77で+4件: @image_preview全4シナリオPASS
  - pending 16件: @anchor_popup 4 + @post_number_display 3 + @pagination ポーリング2 + 旧7件 — 全て意図的Pending
- playwright E2E smoke (navigation): 19テスト / 全PASS
...
- **本番スモークテスト (Sprint-77後):** 23/24 PASS（1件skip: ローカル限定認証テスト）
