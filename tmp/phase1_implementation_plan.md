...
Step 1〜7では単体テスト（vitest）のみで品質を担保し、BDDステップ定義は未実装のまま進めた。
これはBDDのベストプラクティス（実装と同時にシナリオをGreenにする）から外れており、仕様解釈のズレが後で発覚するリスクがある。

**方針転換（Sprint-8以降）:**
- **Step 7.5（BDD負債返済）**: Step 1〜7に対応するBDDステップ定義を一括実装し、`npx cucumber-js` でGreenにする。Step 8着手前に完了させる
- **Step 8以降**: 各Stepの完了条件に「対応するBDDシナリオが `npx cucumber-js` でPASS」を含める。実装と同時にステップ定義を書く
...
- `supabase migration new create_tables` → `supabase/migrations/{ts}_create_tables.sql`: threads, posts, users, currencies, bots, bot_posts, accusations, incentive_logs, auth_codes, admin_users
- `supabase migration new create_indexes` → `supabase/migrations/{ts}_create_indexes.sql`: architecture.md §11.2 のインデックス定義
- `supabase migration new rls_policies` → `supabase/migrations/{ts}_rls_policies.sql`: architecture.md §10.1.1 のRLSポリシー
- 適用: `supabase db push`
...
### Step 7.5: BDD負債返済（Step 1〜7のステップ定義一括実装）★必須
- Step 1〜7で実装済みの機能に対応するBDDステップ定義を `features/step_definitions/` に作成
- 対象feature: `authentication.feature`, `posting.feature`, `thread.feature`, `currency.feature`, `incentive.feature` 等（Step 1〜7に対応する範囲）
- **完了基準**: 対応する全シナリオが `npx cucumber-js` でPASS
- **依存**: Step 7（Step 1〜7の実装が揃っていること）
- **注意**: このStepが完了するまでStep 8以降に進まないこと

### Step 8: 管理機能（Step 9, 10 と並行可）
...
- **テスト**: vitest 単体テスト + `admin.feature`, `authentication.feature` の管理者シナリオのBDDステップ定義
- **完了基準**: `npx vitest run` 全PASS + 対応シナリオが `npx cucumber-js` でPASS
- **依存**: Step 7.5

### Step 9: 専ブラ互換 Adapter（Step 8, 10 と並行可）
...
- **テスト**: vitest 単体テスト + `specialist_browser_compat.feature` のBDDステップ定義
- **完了基準**: `npx vitest run` 全PASS + 対応シナリオが `npx cucumber-js` でPASS
- **依存**: Step 7.5
...
- 日次リセットIDの残シナリオ
- **テスト**: vitest 単体テスト + `mypage.feature`, `authentication.feature` の全シナリオのBDDステップ定義
- **完了基準**: `npx vitest run` 全PASS + 対応シナリオが `npx cucumber-js` でPASS
- **依存**: Step 7.5
...
Step 0 (基盤)
  ...
  └── Step 2 (型)  ──┴── Step 3 (リポジトリ)
                     ...
                     Step 7 (Web UI)
                            │
                     Step 7.5 (BDD負債返済) ★ここでStep1〜7のステップ定義を一括実装
                            │
            ┌───────────────┼───────────────┐
     Step 8 (Admin)   Step 9 (専ブラ)   Step 10 (マイページ)
     ※各StepにBDDステップ定義を含む（実装と同時にGreenにする）
            └───────────────┼───────────────┘
                            │
                     フェーズ5: 検証サイクル (bdd-gate / code-review / doc-review)
```
